...
## Phase 20: PostgreSQL統合テスト (4 steps, 完了済み)

### Step 20.1: PostgreSQLビルドシステム連携 (完了)
- `-E`, `-S`, `-c` モードのCLI対応
- `-I`, `-D` フラグでのインクルードパスとマクロ定義
- マルチファイルコンパイルとリンク

### Step 20.2: PostgreSQL基本ヘッダーのコンパイル (完了)
- `postgres.h`, `fmgr.h`, `utils/builtins.h` のコンパイル
- `PG_MODULE_MAGIC` マクロ（静的構造体初期化子）対応
- `PG_FUNCTION_INFO_V1` マクロ対応

### Step 20.3: 前方宣言構造体の解決とSPIヘッダーのコンパイル (完了)
- `TypeKind::Struct` にタグ名を追加して正確な前方参照追跡
- O(n²)の即時更新から、タグベースの遅延解決に変更
- `executor/spi.h` (29K行) のコンパイル成功
- 自己参照構造体（`struct Node { Node *next; }`）の正しい処理

### Step 20.4: PICコード生成と共有ライブラリサポート (完了)
- `-fPIC` フラグによるPICコード生成
- extern変数のGOT経由アクセス (`@GOTPCREL`)
- 関数呼び出しのPLT経由 (`@PLT`)
- `-shared` フラグによる共有ライブラリリンク
- PostgreSQL拡張モジュール(.so)の生成確認

---

## Phase 21: GCCプリプロセッサ互換性とPostgreSQLバックエンド完全コンパイル (3 steps)

### Step 21.1: GCCプリプロセッサ互換性 — パーサー修正 (完了)
- extern関数定義 (`extern type func() { body }`) のパース
- 非extern カンマ区切りグローバル変数宣言
- GCC文式 `__extension__ ({...})` のキャスト/文脈対応
- staticローカル変数の波括弧初期化子の二重消費バグ修正
- **結果: 331/331 PostgreSQLバックエンドファイル パース成功 (100%)**

### Step 21.2: コード生成の修正とアセンブル検証
- 実際にアセンブルが通るかの検証 (`gcc -c` でオブジェクトファイル生成)
- コード生成の問題修正
- アセンブルエラーの解消

### Step 21.3: リンクと動作検証
- PostgreSQL拡張モジュールのリンク検証
- 基本的なPostgreSQLユーティリティ関数のコンパイル・リンク・実行確認

---

## 合計: 139+ steps across 21 phases
